-- ============================================================================
-- 店家自開團的訂單不能轉出（不要的單就取消／刪掉）
-- ============================================================================
-- 老闆指示（2026-08-31）：「店家產的訂單不能再轉出了，很簡單就是刪除。」
--
-- 為什麼一定要擋（不是偏好問題，是會壞）：自開團的帳整個綁在 (團, 主辦店) 上 ——
--   * _store_campaign_demand 只算 pickup_store = 主辦店 的需求
--   * 取貨閘門 Path S 要求 gc.owner_store_id = co.pickup_store_id
-- 而轉單是「把品項複製到另一張單」，跨店轉還會落在別家店。轉出去的那張單因此
-- **永遠不會被算進收貨頁的需求、也永遠過不了取貨閘門** —— 貨在店裡、客人卻
-- 領不到，而且畫面不會報錯，就是 CLAUDE.md 記過的那種安靜卡死。
-- 同店換客人也一樣：新單的 campaign_id 沿用來源單，但它是另一張單，
-- 來源單的品項被標 cancelled，需求就這樣憑空少掉一塊。
--
-- 所以兩支轉單 RPC 一律擋下自開團的來源單，並在錯誤訊息直接告訴店家怎麼做。
--
-- 連帶必須放寬的一處：rpc_delete_order_item 的狀態閘排除 confirmed，理由寫的是
-- 「confirmed 仍被 PR 鎖定」。自開團從來不進請購單（20260831000060 的兩個路口
-- trigger 讓它結構上進不去），而**結單後它正好停在 confirmed** ——
-- 不放寬的話，轉單擋掉之後店家會完全沒有出路（整單取消可以，單獨刪一項不行）。
-- 只對自開團放行，總倉團的 PR 鎖定規則一字不動。
--
-- 三支都是逐字取自線上定義後只插入守衛／只改那一個 IF，其餘一行未動：
--   rpc_transfer_order_to_store / rpc_transfer_order_partial / rpc_delete_order_item
-- Rollback：三支各自 CREATE OR REPLACE 回線上改動前的版本
--   （本檔套用前的 prosrc 就是基底，內容與本檔差在上述守衛）。
-- ============================================================================
...
BEGIN
  ...
  -- 20260831：店家自開團的訂單不能轉出。
  --   轉單是「把品項複製到另一張單」（跨店轉還會落在別家店），而自開團的帳
  --   整個綁在 (團, 主辦店) 上：_store_campaign_demand 只算
  --   pickup_store = 主辦店 的需求，取貨閘門 Path S 也要求
  --   gc.owner_store_id = co.pickup_store_id。轉出去的那張單因此永遠不會被算進
  --   收貨頁的需求、也永遠過不了閘門 —— 貨在店裡、客人卻領不到，而且畫面不會
  --   報錯（CLAUDE.md 記過的那種安靜卡死）。
  --   老闆指示：不要再想辦法讓它轉，不要的單就直接取消／刪掉。
  IF EXISTS (
    SELECT 1
      FROM customer_orders co
      JOIN group_buy_campaigns gc ON gc.id = co.campaign_id
     WHERE co.id = p_order_id
       AND gc.owner_store_id IS NOT NULL
  ) THEN
    RAISE EXCEPTION '門市團的訂單不能轉出：這團的貨是門市自己採購、自己收貨的。客人不要的話請直接取消訂單或刪掉品項';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.rpc_transfer_order_to_store TO authenticated;

CREATE OR REPLACE FUNCTION public.rpc_transfer_order_partial(p_order_id bigint, p_to_pickup_store_id bigint, p_to_member_id bigint, p_to_channel_id bigint, p_operator uuid, p_reason text, p_items jsonb, p_is_air_transfer boolean DEFAULT false, p_aid_board_id bigint DEFAULT NULL::bigint)
 RETURNS bigint
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
GRANT EXECUTE ON FUNCTION public.rpc_transfer_order_partial TO authenticated;

CREATE OR REPLACE FUNCTION public.rpc_delete_order_item(p_order_id bigint, p_item_id bigint, p_operator uuid, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- 20260831：店家自開團的 confirmed 也放行。
  --   confirmed 被排除的理由是「已被 PR 鎖定」，但自開團從來不進請購單
  --   （20260831000060 的兩個路口 trigger 讓它結構上進不去），而結單後它正好
  --   停在 confirmed —— 不放行等於「結完單就一項也刪不掉」，而轉單又已經擋掉，
  --   店家會完全沒有出路。總倉團的 PR 鎖定規則一字不動。
  IF v_order.status NOT IN ('pending', 'ready', 'partially_completed')
     AND NOT (
       v_order.status = 'confirmed'
       AND EXISTS (
         SELECT 1 FROM group_buy_campaigns gc
          WHERE gc.id = v_order.campaign_id AND gc.owner_store_id IS NOT NULL
       )
     ) THEN
...
GRANT EXECUTE ON FUNCTION public.rpc_delete_order_item TO authenticated;

COMMENT ON FUNCTION public.rpc_transfer_order_to_store(BIGINT,BIGINT,BIGINT,BIGINT,UUID,TEXT,BOOLEAN) IS
  '整單轉給別人／別店。20260831 起擋下店家自開團的來源單（轉出去的單過不了取貨閘門），'
  '請改用取消訂單 / 刪除品項。';
COMMENT ON FUNCTION public.rpc_transfer_order_partial(BIGINT,BIGINT,BIGINT,BIGINT,UUID,TEXT,JSONB,BOOLEAN,BIGINT) IS
  '部分轉出。20260831 起擋下店家自開團的來源單（理由同整單轉出）。';
COMMENT ON FUNCTION public.rpc_delete_order_item(BIGINT,BIGINT,UUID,TEXT) IS
  '刪除訂單品項（軟刪 → cancelled）。20260831 起店家自開團的 confirmed 訂單也放行：'
  '它不進請購單，結單後停在 confirmed，不放行就一項也刪不掉。總倉團規則不變。';
