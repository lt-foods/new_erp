-- ============================================================================
-- 2026-08-28: 撤銷收貨也只對「有本批 SKU」的 ready 訂單重驗閘門
--
-- 昨天 20260827000000 修的是收貨（邏輯 C）；撤銷收貨（rpc_unreceive_transfer）
-- 的反向邏輯 C 有一模一樣的問題，昨天沒一起修：
--
--   FOR v_ord IN SELECT id FROM customer_orders
--     WHERE pickup_store_id = v_dest_store_id AND status = 'ready'   -- ← 整店
--   LOOP IF NOT public.is_order_pickup_ready(v_ord.id) THEN ...
--
-- 閘門 ~8ms/張 × 該店全部 ready 單：中和 2,280 張 ≈ 18 秒、三峽 1,217 張、
-- 古華 788 張 —— 線上 8/28 06:44 實測 **10,481ms**（古華 WAVE-2221-S41，
-- 本批只有 1 個品項），而 06:44 與 07:06 兩次 DB 非正常關閉前都剛跑過這支。
-- 升到 Small(2GB) 之後仍會當，所以這支是還沒拆掉的另一顆同型未爆彈。
--
-- 語意：撤銷收貨只把**本批 SKU** 的貨退回在途，沒有這些 SKU 的訂單閘門結果
-- 與撤銷前相同（撤銷前是 ready ＝閘門 true，撤銷後仍 true），不會被退回
-- shipping，濾掉不改變任何單的最終狀態。aid 單分支（v_customer_order_id
-- IS NOT NULL）本來就只看一張單，不動。
--
-- 基底版本：線上 pg_get_functiondef() 現況（2026-08-28 取出，與
--   20260824030000_unreceive_withdraw_hq_reports.sql 逐字一致，已比對），
--   只改反向邏輯 C 那一段。
-- Rollback：把 v_gate_candidates 前濾拿掉、迴圈條件退回
--   `pickup_store_id = v_dest_store_id AND status = 'ready'`（＝20260824030000）。
--
-- 正式庫 A/B 對拍（中和店 2,280 張 ready、transfer 14883，rollback）：
--   28,326ms → 102ms（278 倍）。退回的訂單集合相差一張：OV-45-0001（抵減
--   容器單，閘門在撤銷**之前**就是 false、且無本批 SKU 的 active 品項）——
--   舊版整店掃描把它「順手」退回 shipping，新版不動它。這不是回歸而是
--   誤傷修正：#852 才剛規定波次推進不准動抵減單，而容器單被退出 ready
--   會讓 _trim_internal_pool 的池子口徑漏看它。其餘 6 張（真的因本批
--   SKU 而不再到貨的）兩版完全一致。
--
-- 沒有對應的前端改動。
-- ============================================================================
