-- ============================================================
-- 2026-08-13: 轉單 TF 序號改 MAX(既有尾碼)+1 —— COUNT(*)+1 會重發已用過的單號
--
-- 事故（2026-08-13 湖口店，轉出訂單 RR-435 → 楊麗君）：
--   確認轉出直接跳 duplicate key value violates unique constraint
--   "customer_orders_tenant_id_order_no_key"，整個轉出動作做不下去。
--
-- 根因：兩支轉單 RPC 的新單號是
--     SELECT COUNT(*) + 1 ... WHERE campaign_id = v_orig.campaign_id
--   → campaign_no || '-TF' || lpad(seq, 4, '0')
--   但 rpc_delete_restock_request（20260714000020/30/40）會「硬刪」RR- 內部單，
--   而全部 RR- 單 + 它們轉出的 TF 單都掛在同一個 __INTERNAL_RESTOCK__ 團底下
--   → 刪過幾張申請之後 COUNT 倒退到既有最大 TF 序號後面，下一次轉出就重發
--   已用過的號碼。線上實測（2026-08-13）：該團 446 張單、MAX TF 序號 447，
--   COUNT+1 = 447 → __INTERNAL_RESTOCK__-TF0447 已存在，必撞。
--   一般團購團沒有刪單路徑、平常撞不到，但 COUNT 式序號同樣擋不住
--   「兩張不同來源單同時轉出」的平行（來源單的 advisory lock 各鎖各的）。
--
-- 修正（兩支同步改，只動序號那一段）：
--   1. 序號改「該團既有 -TF 尾碼最大值 + 1」：單調遞增、不受硬刪影響。
--   2. 算號前先 pg_advisory_xact_lock('order_tf_seq:' || campaign_id)：
--      序號產生在同團內序列化（鎖順序固定為 來源單鎖 → 團鎖，不會互等）。
--   3. lpad 寬度改 GREATEST(len, 4)：lpad 超過指定寬度會「截斷」，
--      seq 破萬時 10001 會被砍成 1000 又撞號，順手拆掉這顆雷。
--
-- 基底版本：20260810010000_transfer_copy_active_items_only（2026-08-13 以
--   pg_get_functiondef 取回線上定義比對，無 drift）。
-- Rollback：CREATE OR REPLACE 回 20260810010000 內的兩支函式定義。
-- ============================================================
...
COMMENT ON FUNCTION public.rpc_transfer_order_to_store(
  ...
  '不受 RR- 單硬刪影響。錯誤訊息繁體中文。基底 20260810010000。';
