-- ============================================================
-- Hotfix: 上一版誤用 prices.is_active（不存在欄位）
-- 改為依 effective_from DESC 取最新
-- ============================================================
...
BEGIN
  ...
  -- 5. items（含售價/分店價 snapshot；prices 用 effective_from DESC 取最新）
...
-- 同步：edit page 也會用既有 prices 做 backfill；現有 PR 補
UPDATE purchase_request_items pri
   SET retail_price = COALESCE(retail_price, (
         SELECT price FROM prices
          WHERE sku_id = pri.sku_id AND scope = 'retail'
          ORDER BY effective_from DESC NULLS LAST
          LIMIT 1
       )),
       franchise_price = COALESCE(franchise_price, (
         SELECT price FROM prices
          WHERE sku_id = pri.sku_id AND scope = 'franchise'
          ORDER BY effective_from DESC NULLS LAST
          LIMIT 1
       ))
 WHERE retail_price IS NULL OR franchise_price IS NULL;
