-- ============================================================
-- 2026-07-13: 月結應收改以「分店價」計 — 總倉出給分店、跟分店收分店價
--
-- 需求（使用者）：「兩個價格都要，龍潭是總倉，所以出給分店的要跟他們收分店價」。
--
-- 前情（20260715000000 雙口徑版）：
--   cost_amount / branch_amount 已分開計算，但 payable_amount（confirm 後
--   入 store_receivables 的請款金額）仍沿用舊的成本價口徑 — 等於總倉
--   原價轉給分店、零毛利。
--
-- 修法：payable_amount 改填分店價口徑（= branch_amount）。
--   - cost_amount / branch_amount 雙口徑照算照存，不動。
--   - rpc_confirm_store_monthly_settlement 不用改：它本來就取
--     payable_amount 入應收單。
--   - 自由轉貨（估價、兩口徑同額）與空中轉（兩店鏡像）行為不變；
--     退貨沖回改以分店價沖（買進付分店價、退回也退分店價，對稱）。
--   - 已 confirmed/settled 的月結不受影響（生成器本來就跳過）；
--     draft 重跑生成器即改用新口徑。
--
-- 基底版本：rpc_generate_hq_to_store_settlement = 20260715000000（線上現行）；
--   逐字複製，僅改 payable 取值（v_payable := 分店價口徑）與 upsert/回傳對應。
-- Rollback：
--   CREATE OR REPLACE 回 20260715000000 版本（payable = 成本口徑）；
--   draft 重跑生成器即回舊口徑。
-- ============================================================

COMMENT ON COLUMN store_monthly_settlements.payable_amount IS
  '該店本月應付總倉（分店價口徑 = branch_amount；20260715000100 起）。'
  '成本口徑參考值見 cost_amount。';
...
COMMENT ON FUNCTION public.rpc_generate_hq_to_store_settlement IS
  ...
  'cost_amount / branch_amount。20260715000100 起 payable = 分店價口徑'
  '（賣斷制：總倉跟分店收分店價）；自由轉貨兩口徑同用估價。'
