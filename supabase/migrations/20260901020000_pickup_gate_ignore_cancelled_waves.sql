-- ============================================================================
-- 取貨閘門 Path C：已取消的撿貨波次不算「對齊波次」
-- ============================================================================
-- 症狀（2026-09-01 平鎮 GRP-20260824-031-0004 / 客人 lris郁云2）：
--   總倉 13:05 收貨完成、庫存 +1、單頭也被 _advance_arrived_confirmed_orders
--   推成 ready（畫面寫「到貨：9/1 下午 1:05」），但同一張單的 4 個品項全部
--   顯示「⏳ 未到貨」「尚有 4 件店裡沒貨，無法取貨」，客人端也是未到貨。
--   店員只能改走轉單（＝ CLAUDE.md 記過的重複單災情），或請人開減抵單繞過
--   （當天 18:37 就是這樣用 4 張 DN 硬開的）。
--
-- 成因：同一批貨有兩支波次
--   WV260901002512（campaign_id=4302 的團購行）→ 11:28:11 **取消**，沒出貨
--   WV260901002513（同 SKU 但走補貨行、campaign_id = NULL）→ 11:28:23 出貨、13:05 收貨
--   Path B 要「campaign 對齊的波次有實收」→ 對齊的那支被取消、連 transfer 都沒有 → false
--   Path C 是「完全沒有對齊波次才退用本店實收」→ 被**已取消**波次的殘留列擋住 → false
--   兩條路一起關上 = 貨在店裡卻發不出去。
--
-- 修法：Path C 的「有沒有對齊波次」只看還活著的波次（pw.status <> 'cancelled'）。
--   取消的波次一件貨都沒出（全站 cancelled 波次的 generated_transfer_id 全為 NULL），
--   本來就不該代表「這組的貨會由那支波次送來，先別走退路」。
--   Path B 不動：它本來就 JOIN generated_transfer_id，取消的波次接不上。
--   兩道數量守衛（campaign-local 供給 / 門市實體 on_hand）原封不動 ——
--   放行的仍然只有「貨真的還在架上」的行，老早被領走的舊單不會被這支打開。
--
-- 影響面（套用前實測）：全站 39 組 (團,店,SKU) 的對齊波次全數被取消，
--   其中「店裡實際收過貨、卻被閘門擋著」的活品項 26 筆／10 家店。
--
-- 基底版本（逐字抽出，只改 Path C 的 NOT EXISTS）：
--   20260831000020_store_self_campaign_pickup_gate.sql
-- Rollback：CREATE OR REPLACE 回 20260831000020 的版本。
-- ============================================================================
...
COMMENT ON FUNCTION public.is_order_item_pickup_ready(BIGINT) IS
  ...
  '（2026-09-01 起「對齊波次」不含已取消的波次）/ D 庫存減抵單（**逐品項**）/ '
