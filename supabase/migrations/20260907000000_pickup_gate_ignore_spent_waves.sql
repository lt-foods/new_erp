-- ============================================================================
-- 取貨閘門 Path C：已結案且該 SKU 零實收的撿貨波次，不算「對齊波次」
-- ============================================================================
-- 症狀（2026-09-07 南平店 GRP-20260709-007「一次性加厚乾髮帽」，連帶 7 家店）：
--   後台開團頁 5 張單全部標「可取貨」、單頭 status = 'ready'（8/31 收貨當下推的），
--   店員在取貨頁卻**勾不到那個品項**，會員端「待取貨」也是空的（南平店 19:03
--   回報：271007 已取貨完成、狀態依然呈現可取貨；528097 / 877818 / 787057 /
--   824829 臨櫃取貨時「一次性加厚乾髮帽」不會出現在可取貨商品中）。
--   貨真的在店裡：南平 on_hand = 25，剛好等於這 5 張單的未取量。
--
-- 成因：同一組貨派了兩次
--   WV260807001197（campaign_id = 2949 的團購行）→ WAVE-1053-S42 派 25、
--     8/20 收貨**實收 0**（全短少），8/21 總倉以 restock_hq 沖回總倉庫存結案
--   WV260828002405（同 SKU 但走**補貨行**、campaign_id = NULL）→ WAVE-2261-S42
--     8/28 出貨、8/31 收貨實收 25（貨就是這批進來的）
--   Path B 要「campaign 對齊的波次有實收」→ 對齊的那支實收 0 → false
--   Path C 是「完全沒有對齊波次才退用本店實收」→ 被那支**已經結案、一件都沒到**
--     的波次殘留列擋住 → false
--   兩條路一起關上 = 貨在店裡卻發不出去。與 20260901020000（取消的波次）同型，
--   只是這次擋路的波次不是 cancelled，而是「跑完了但沒送到」。
--
--   單頭之所以還是會變 ready：8/31 店家走收貨頁的手動配單勾了這幾張單，
--   rpc_receive_transfer_manual 對「勾選的 shipping 單」是直接推 ready 的
--   （不過閘門）。所以畫面寫可取貨、取貨頁打不開，兩邊各說各話。
--
-- 修法：Path C 的「有沒有對齊波次」再排除一種波次 —— 已經派出、而且調撥單
--   已經 received / closed / cancelled、該 SKU 實收合計 0。調撥單進了這三個狀態
--   就不會再有實收（要改只能先退回收貨），Path B 對它永遠開不了；留著只會把
--   Path C 這條退路堵死。**還沒派（generated_transfer_id IS NULL）或在途
--   （draft / confirmed / shipped）的波次維持擋住**，那才是真的「貨還在路上」。
--   Path B 不動；兩道數量守衛（campaign-local 供給 / 門市實體 on_hand）原封不動
--   —— 放行的仍然只有「貨真的還在架上、而且輪得到這一行」的品項。
--
-- 影響面（套用前對線上實測）：全站 6,109 組 (團,店,SKU) 裡，對齊波次全部
--   「已結案零實收」的有 193 組，其中店裡真的收過這個 SKU（Path C 後半成立）的
--   13 組 / 8 家店 / 40 個活品項；套用前後逐筆對拍（新版在交易裡替換掉函式、
--   跑完比對再 ROLLBACK），閘門由 false → true 的是 32 個品項 / 10 組 / 7 家店，
--   沒有任何一筆由 true → false（新條件只會從 NOT EXISTS 的母體「拿掉」列，
--   Path C 只可能由關變開）。on_hand = 0 的 3 組仍被門市實體庫存守衛擋著（正確），
--   古華 GRP-20260629-011 也有 1 筆因為排在後面、庫存輪不到而維持關閉。
--
-- 已於 2026-09-07 以 Management API 套上正式庫（套用後南平那 5 個品項閘門 = true，
--   全站放行 32 個品項；取貨頁 / 會員端讀的是 v_order_item_pickup_ready 與
--   v_customer_order_summary，兩支都直接呼叫本函式 → 不需要前端部署）。
--
-- 基底版本（逐字抽出，只改 Path C 的 NOT EXISTS）：
--   20260901020000_pickup_gate_ignore_cancelled_waves.sql
-- Rollback：CREATE OR REPLACE 回 20260901020000 的版本。
-- ============================================================================
           ...
           (
             NOT EXISTS (
               SELECT 1 FROM picking_wave_items pwi
                 ...
                 LEFT JOIN transfers wt ON wt.id = pwi.generated_transfer_id
                WHERE pwi.tenant_id   = co.tenant_id
                  AND pwi.campaign_id = co.campaign_id
                  AND pwi.store_id    = co.pickup_store_id
                  AND pwi.sku_id      = coi.sku_id
                  -- 2026-09-01：已取消的波次不算「對齊波次」。取消的波次一件貨都沒出
                  --（全站實測：cancelled 波次的 picking_wave_items.generated_transfer_id
                  --  100% 是 NULL），留著只會把 Path C 這條退路堵死。
                  AND pw.status <> 'cancelled'
                  -- 2026-09-07：**已經跑完、這個 SKU 一件都沒進來**的波次同理。
                  --   調撥單一旦 received/closed/cancelled 就不會再有實收（要改只能
                  --   先退回收貨），所以它的 qty_received = 0 是定局：Path B 永遠
                  --   開不了，留著只會跟取消的波次一樣把 Path C 堵死。總倉的補派
                  --   走補貨行（picking_wave_items.campaign_id = NULL），接不回這一組。
                  --   還沒派（generated_transfer_id IS NULL）或在途（draft/confirmed/
                  --   shipped）的波次**維持擋住** —— 那是真的「貨還在路上，先別走退路」。
                  AND NOT (
                    pwi.generated_transfer_id IS NOT NULL
                    AND COALESCE(wt.status, 'cancelled') IN ('cancelled','received','closed')
                    AND COALESCE((
                      SELECT SUM(ti2.qty_received)
                        FROM transfer_items ti2
                       WHERE ti2.transfer_id = pwi.generated_transfer_id
                         AND ti2.sku_id      = coi.sku_id
                    ), 0) = 0
                  )
             )
             AND EXISTS (
...
COMMENT ON FUNCTION public.is_order_item_pickup_ready(BIGINT) IS
  ...
  '（2026-09-01 起「對齊波次」不含已取消的波次；2026-09-07 起也不含已結案而該 SKU '
  '零實收的波次）/ D 庫存減抵單（**逐品項**）/ '
  'D'' offset 抵減單 / S 店家自開團該團的入庫異動，'
  '外加兩道數量守衛：campaign-local 供給、門市實體 on_hand（後者含 DN 覆蓋的等貨行）。'
  '基底 20260901020000_pickup_gate_ignore_cancelled_waves。';
