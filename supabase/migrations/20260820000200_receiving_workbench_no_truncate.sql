...
--
-- 結果集與 v2 的關係 ── ⚠ 有前提，**不是無條件成立**（2026-08-20 審查修正）：
--   * sent / partially_received：v3 全取 ⇒ v2 有的一定有。無條件成立。
--   * fully_received：v2 取的是「三種狀態混在一起」的 sent_at top-200；同一張單
--     在「只看 fully_received」的排序裡名次只會往前 ⇒ 仍在 top-200 內 → 必含。
--     **但這一步只在 sent_at 沒有並列卡在第 200 名邊界時才嚴格成立。**
--
--   ⚠ 並列時為什麼證不出來：v2 的 `ORDER BY sent_at DESC NULLS LAST` 也沒有
--     tie-breaker（20260819000000:51-57）⇒ 「v2 究竟會挑哪 200 張」這件事
--     **在 SQL 語義上根本沒有定義**。拿一個沒有定義的集合當被涵蓋方，任何版本
--     都不可能宣稱嚴格超集 —— 這不是本檔改壞了什麼，是 v2 本來就有的性質。
--     本檔在 fr_recent 加 `, p.id DESC` 能買到的是：**v3 自己每次回傳同一批**
--     （否則同一台 iPad 連按兩次重整，「已收」清單可能換一批），
--     以及讓下面那份驗收 SQL 的比對是可重現的、不是抽一次樣。
--
--   ⇒ 「舊畫面一張不掉」**改用正式庫真資料證實，不靠上面這段推論**：
--     D:\1人公司\公司\01_進行中\驗收_進貨待辦不截斷_2026-08-20.sql
--       ② 把 v2 的 po CTE 原文內嵌，重建舊 top-200，逐 id 比對有沒有人消失；
--       ③ 再算一次「所有可能的 v2 top-200」的聯集（rank() ≤ 200），
--          連「換一種並列挑法會不會掉」都一起排除掉。
--     ③ 全綠 ＝ 數學上保證；②全綠而③不綠 ＝ 這次沒掉、但邊界上有並列。
--
...
--     平均 7（21,210 列）  200→475   10.8 ms   16.2 ms   +5.4 ms (+50%)
--     平均 27（81,926 列） 200→475   29.1 ms   49.5 ms   +20.4 ms (+70%)
--
--   ⭐ 拆開看 —— 這一版做了兩件事，代價幾乎全在第二件：
--     ①「未收完不再截斷」本身幾乎免費：畫面上只剩未收完的 275 列時，
--        10.9 / 32.8 ms ＝ +1% / +13%（7 行那組落在量測雜訊內）。
--     ②「已全收的額度」從 0 一路開到 200（275→475 列）才是主要成本，
--        而那是上面說的「不能調小」換來的。
--        中間刻度（27 行那組）：50 → 325 列 37.0 ms；100 → 375 列 40.4 ms。
--     ⚠ ① 的基準是「已全收額度 **0**」（harness 變體 `v3_0`，見 run.mjs
--       的 VARIANTS）。舊版註解把它寫成「從 100 拉到 200」，與變體定義
--       對不起來、數字鏈兜不攏，2026-08-20 審查抓到後改正。
--     以上數字全部出自同一輪 harness（12 回合中位數），兩組品項行數各跑一次。
...
BEGIN
  ...
  -- ⛔ 這裡刻意只比對 `product_names` 這個字串，**不要**改成比對整段定義或雜湊。
  --   本檔要擋的是「有沒有跳過 20260819000000」這一件事，而 product_names 正是
  --   那一支唯一新增的東西 ⇒ 這個標記回答的就是那個問題。
  --   改成定義雜湊會壞掉：pg_get_functiondef 回的是 Postgres 重新排版過的文字
  --   （空白、大小寫、型別寫法都會被正規化），跟 migration 檔裡的原文不會逐字相同
  --   ⇒ 正牌的 v2 也會被擋下來，把「順利部署」變成「當場卡死」。
  --   誤放行的前提是「有一支不是 v2、卻剛好含 product_names 的函式」—— 那種函式
  --   既然吐得出 product_names，前端要的鍵就在，風險遠小於誤擋。
...
AS $$
  WITH fr_recent AS (
    ...
    -- ⚠ `, p.id DESC` 是 tie-breaker，不是排版。只有 sent_at 一個鍵時，
    --   一堆單同時間送出、又剛好卡在第 200 名邊界的話，Postgres 挑哪一張
    --   **不保證**（同一份資料換個計畫就換一批）⇒ 同一台 iPad 連按兩次重整，
    --   「已收」清單可能不一樣。id 是 BIGSERIAL 主鍵（20260422120004:70），
    --   接上去就是嚴格總序，每次回傳同一批。
    SELECT p.id
    FROM purchase_orders p
    WHERE p.status = 'fully_received'
    ORDER BY p.sent_at DESC NULLS LAST, p.id DESC
    LIMIT 200
  ),
  po AS (
