-- ============================================================
-- rpc_create_order_return：分店主動發起「退訂單回總倉」
--
-- 場景：分店收到顧客訂單派的貨後（status: ready/shipping/partially_completed/completed），
--   要把全部 / 部分退回總倉。必須關聯 customer_order，且 SKU + 數量必須在
--   該訂單實際派到該店的 transfer_items.qty_shipped 範圍內、扣掉已退量。
--
-- 對比：
--   - rpc_create_free_transfer：自由轉貨、虛擬 SKU、店↔店、無訂單關聯（已禁掉店→總倉方向）
--   - rpc_reject_transfer：dest 端拒收（reverse 出貨方向、自動取消訂單）
--   - 本 RPC：dest 端「貨已收、要退回」（正向出貨、訂單狀態保留、不取消）
--
-- TEST: docs/TEST-order-return.md
-- ============================================================

CREATE OR REPLACE FUNCTION public.rpc_create_order_return(
  p_order_id BIGINT,
  p_lines    JSONB,    -- [{ sku_id, qty, notes? }]
...
BEGIN
  ...
  END IF;
  IF v_order.status NOT IN ('shipping','ready','partially_completed','completed') THEN
    RAISE EXCEPTION 'order % status=% cannot be returned (must be shipping/ready/partially_completed/completed)',
  ...
  -- 6. 建退貨 transfer 單頭（status=shipped 直接記出貨）
  ...
  -- 7. 每行：驗證 + outbound + 建 transfer_items
  ...
  LOOP
    ...
    -- 7a. 該訂單該 SKU 量：以 customer_order_items.qty 為準
    -- （正常訂單預留庫存即在店；aid 訂單 customer_order_items.qty = transfer qty。
    --  customer_order_items 是 single source of truth、簡單一致。）
    ...
    -- 7b. 已退量（先前的 return_to_hq transfer，本筆 transfer 還沒含進來）
    SELECT COALESCE(SUM(ti.qty_shipped), 0) INTO v_already_ret
     ...
     WHERE t.customer_order_id = p_order_id
       ...
       AND t.id <> v_transfer_id  -- 排除剛剛新建的
    ...
    -- 7c. 出庫 (movement_type='customer_return')
...
GRANT EXECUTE ON FUNCTION public.rpc_create_order_return(BIGINT, JSONB, TEXT, UUID)
  TO authenticated;

COMMENT ON FUNCTION public.rpc_create_order_return IS
  '分店主動發起退訂單回總倉。建 return_to_hq transfer (status=shipped) + 出庫扣店端庫存。SKU 與數量必須在 customer_order 已派該店量範圍內。';
