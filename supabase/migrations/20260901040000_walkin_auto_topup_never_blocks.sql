-- ============================================================================
-- 現場銷售：缺貨自動補帳，結帳一律過（拿掉數量閘門與畫面上的補貨勾選）
-- ============================================================================
-- Alex 2026-09-01：
...
--
-- 為什麼「不擋」是對的：
--   客人就站在櫃台前、東西在店員手上，貨**一定會**離開這家店。系統擋下來不會讓
--   貨留下，只會讓帳跟現實脫節 —— 店員繞過系統手收現金，那筆交易就完全消失
--   （沒有銷售、沒有扣庫存、沒有日結）。把它記成「+N 入帳 → −N 賣掉」才是那一刻
--   真正發生的事。同 CLAUDE.md 空中轉出庫 `p_allow_negative => TRUE` 的理由：
--   「貨實際上離開轉出店了，記下這筆出庫比拒絕記錄準確」。
--
--   走 +N/−N 而不是負庫存，是因為 on_hand 被全站好幾道閘門讀著
--   （取貨閘門的實體守衛、可分配量、現貨直配…），讓它變負數會誤擋別人的取貨。
--
-- 代價與配套：
--   帳面差異被抹平（on_hand 淨變化 0），所以**紀錄就是唯一的訊號**。
--   每一筆自動補帳都寫 manual_adjust ＋ reason 帶單號，`/pos/topups` 是那份稽核
--   清單（同店連續補 3 天以上會跳提示導到盤點）。**刪掉那支報表＝這個口就沒人看著了。**
--
-- 本檔改動（rpc_create_walkin_sale）：
--   1. 補的量改成 `GREATEST(need − 可賣量, 0)` —— 由系統算，不再看
--      p_lines[].add_stock_qty（該欄位保留只為了舊版前端送過來時不報錯）。
--   2. 拿掉「可賣量不足 → RAISE」整段。數量不再是結帳的阻擋條件。
--   3. 拿掉補帳的角色守衛。一旦自動化，擋 store_staff ＝「店員結不了帳」，
--      而結不了帳的後果是貨照樣出去、系統上沒有這筆。操作人記在 movement 上。
--
-- 仍然會擋的（都不是數量問題）：
--   分店帳號賣別店的貨（wrong_store）、單價 ≤ 0、空購物車、
--   賣給【內部】店容器帳號、不可新增訂單的會員。
--
-- 可賣量（_sku_walkin_qty，20260901020000）**維持不變**，只是語意從「上限」變成
--   「不用補帳就賣得掉的量」—— 畫面拿它顯示「貨不足」與「會自動補 N 件」。
--
-- 基底版本：rpc_create_walkin_sale = 20260901030000（逐字抽出後只改補帳段、
--   刪數量閘門，非手抄）。
-- Rollback：重跑 20260901030000 的 rpc_create_walkin_sale（會把數量閘門與
--   店長守衛一起裝回去）。
-- ============================================================================
...
COMMENT ON FUNCTION public.rpc_create_walkin_sale(BIGINT, JSONB, UUID, BIGINT, TEXT, TEXT, NUMERIC, TEXT) IS
  ...
  '成交後扣【內部】店已到貨現貨池。20260901040000。';
