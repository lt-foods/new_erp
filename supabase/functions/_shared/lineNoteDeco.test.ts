...
Deno.test("標記過的金額換成 💲＋全形數字，沒標記的一個字不動", () => {
  eq(applyDeco(`一份 ${decoPrice("$120")}`), "一份 💲１２０", "基本");
  eq(applyDeco(`${decoPrice("$30")}（市價$150/盒）`), "💲３０（市價$150/盒）", "行內市價維持純文字");
  eq(applyDeco(decoPrice("$99.5")), "💲９９.５", "小數點原樣留著");
...
Deno.test("文案裡自己獨立一行的 $數字、💰 後面的數字才標起來", () => {
  ...
  eq(bag, "A. 原味 💰１９５\n💰 一盒 💲２７５\n💰一包９９元\n💰滿１０００免運", "錢袋後面的數字");
  const full = applyDeco(decoTextPrices("💰＄９９\n＄１２０\n（市價＄１５０/盒）"));
  eq(full, "💰💲９９\n💲１２０\n（市價＄１５０/盒）", "老闆自己打的全形 ＄９９ 也算，行內市價不動");
});
