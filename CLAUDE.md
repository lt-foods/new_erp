...
### 反過來「貨在店裡卻不能取」：先看這組的撿貨波次是不是被取消了

2026-09-01 平鎮 GRP-20260824-031-0004：同一批菜有兩支波次 ——
campaign 對齊的 `WV260901002512` 11:28 被**取消**（一件都沒出），實際出貨的
`WV260901002513` 走補貨行、`picking_wave_items.campaign_id` 是 NULL。於是
Path B（要對齊波次有實收）接不上取消的那支（連 transfer 都沒有），
Path C（**完全沒有**對齊波次才退用本店實收）又被取消波次的殘留列擋住 ——
兩條路一起關上。收貨入庫了、單頭也被 `_advance_arrived_confirmed_orders` 推成
ready（畫面寫「到貨：9/1 下午 1:05」），四個品項卻全是「⏳ 未到貨・店裡沒貨」，
客人端同樣顯示未到貨。店員當下只能開減抵單硬繞（那天 18:37 開了 4 張 DN）。

已修（20260901020000）：Path C 的「有沒有對齊波次」加 `pw.status <> 'cancelled'`。
全站 cancelled 波次的 `generated_transfer_id` 100% 是 NULL ——
取消的波次不代表「等它送來，先別走退路」。套用後放行 28 個品項 / 10 家店，
兩道數量守衛原封不動，on_hand = 0 的舊單一樣打不開。

- `picking_wave_items` 的殘留列在任何「這組有沒有安排出貨」的判斷裡都要濾掉
  已取消的波次：join 了 `picking_wave_items` 就要順手 join `picking_waves` 問 status。
- 診斷順序：`gate = false` 但店裡明明有貨 → 先問這 (團,店,SKU) 的波次是不是
  `bool_and(pw.status = 'cancelled')`，再去看 supplied / on_hand。
- 這是「閘門放行 ≠ 單頭會動」的鏡像：單頭那半推得動（它不看閘門），
  閘門這半卡死 → 畫面自相矛盾（寫著到貨、又寫著未到貨）。

**擋路的波次不只有 cancelled —— 還有「跑完了但一件都沒到」那種。**
2026-09-07 南平 GRP-20260709-007（一次性加厚乾髮帽）：對齊波次 WV260807001197
派 25 件，8/20 收貨**實收 0**、8/21 總倉用 `restock_hq` 沖回總倉結案；總倉 8/28
改走**補貨行**再派一次（`picking_wave_items.campaign_id = NULL`），8/31 收貨實收 25。
Path B 要對齊波次有實收 → 0，Path C 又被那支已結案的殘留列擋住 → 貨在架上
（on_hand 25 = 那 5 張單的未取量）卻取不了；而店家收貨時走**手動配單**勾了那幾張單，
`rpc_receive_transfer_manual` 對勾選的 shipping 單是**直接推 ready、不過閘門**的，
於是開團頁寫「可取貨」、取貨頁勾不到、會員端「待取貨」空的，三邊各說各話。

已修（20260907000000）：Path C 的「有沒有對齊波次」再排除
「已派出且調撥單 received/closed/cancelled、該 SKU 實收合計 0」的波次 ——
調撥單進了那三個狀態就不會再有實收（要改只能先退回收貨），Path B 對它永遠開不了。
**還沒派（`generated_transfer_id IS NULL`）或在途（draft/confirmed/shipped）的維持擋住**。
全站放行 32 個品項 / 10 組 / 7 家店，兩道數量守衛沒動（on_hand = 0 的組照樣打不開）。

- 判斷「這支波次還會不會送來」時，`pw.status <> 'cancelled'` 只答了一半，
  另一半要問它的 `generated_transfer_id` 那張調撥單**收完了沒、收到幾件**。
- 短收後總倉「再派一次」幾乎都會掉 `campaign_id`（補貨行），所以第二批永遠回不到
  原本那一組 —— 凡是依賴「對齊波次」的判斷，都要想一下第二批走的是哪條路。
