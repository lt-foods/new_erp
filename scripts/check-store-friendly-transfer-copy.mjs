...
function stageErrors(source) {
  const grouped = /case "ready":\s*case "partially_completed":\s*case "completed":\s*return "對方店已收貨・轉貨完成";/;
  return grouped.test(source) ? [] : ["轉出店的 ready / partially_completed / completed 必須同一文字"];
}
...
console.log("Store-friendly transfer copy check passed, including 3 mutation counterexamples.");
