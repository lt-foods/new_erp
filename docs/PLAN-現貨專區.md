...
│ 現貨專區            （大標題）│
├─────────────────────────────┤
│  [ 松山店 (3) ][ 全部 (8) ]  │  segmented control（本店在左，預設選中）
├──────────────┬──────────────┤
│ [本店釋出]   │[古華店 釋出] │
│   商品圖     │   商品圖      │
│ 上海小籠湯包 │ 日夜藍莓護眼  │
...
- 分頁：`〈本店名〉` / `全部`（原本分頁下有一段金額規則說明，2026-08-01 拿掉 —— 卡片上的鎖頭已經講得夠清楚）
- **本店那格排在左邊、而且是預設選中的**（2026-08-02 改，原本是「全部」在左且預設）：
  本店的才看得到金額、才真的拿得到貨，跨店的要嘛調貨要嘛只能看，
  所以主要動線放前面、「全部」退成次要。想看別店的自己切過去。
