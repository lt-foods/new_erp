...
畫面上一律用店員讀得懂的說法，函式／欄位名維持英文原詞不跟著改
（改了反而對不上 `_sku_commitment` / `_grow_internal_pool`）：

| 畫面 | 程式 | 白話 |
|---|---|---|
| 待客取 | `promised` | 貨到店了、客人的訂單也掛著，等他來領 |
| 等貨中 | `waiting` | 客人下單了，但貨還沒到 |
| 內部單 | `pool_claimed` | 掛在【內部】xx 店名下的 RR- / OV- 單，要賣走「轉單給客人」 |
| 可分配 | `free` | 沒人認領，可以直接配給客人 |

命名是踩出來的：欄位一度叫「池子 / 已承諾」，使用者反映看不懂
（2026-08-16）。挑選標準是**看到欄名就知道下一步該做什麼** ——
「內部單」讓店員知道去訂單頁找那張單轉出，「待客取」直接說明那批貨在等誰。
...
- 訂單號用 `SP-<店>-<序>`（MAX-based，不用 COUNT+1 —— 單被硬刪會重發撞 unique）。
- **會員端卡片標題**：`orderTitle.ts` 既有的「sentinel 團改顯示品項名」例外
  對新單型自動生效（實測 SP-2-0001 顯示「美魔女棉柔加厚洗臉巾、ins便携手腕…」）✅
- **後台顯示（2026-08-16 回報後補）**：訂單明細的「開團」欄會照實印
  `__INTERNAL_RESTOCK__【內部】補貨申請` —— 一張現貨直配單寫著「補貨申請」，
  店員會看錯。共用 sentinel 是刻意的（濾網現成），代價就落在顯示層，要另外處理：
  - `internalOrderSource(order_no)`（`lib/orderTitle.ts`）：**用單號前綴**判來源
    （`SP-` 現貨直配 / `RR-` 補貨申請 / `OV-` 收貨多給 / `-TF` 轉單轉入 /
    `-INT` 內部叫貨）。不要用 notes 內文比對 —— 單號建立後不會變，notes 會被人改。
  - 套用在：訂單明細（「開團」欄改成「來源」）＋訂單列表（桌機表格／手機卡片
    的開團名那行）。真團一律維持印開團名稱。
- 訂單列表／會員端的金額口徑不用改：這是真客人＋`order_kind='normal'` 的正常單，
...
- 列表欄位「保留 / 可用」→ 換成 **「待客取 / 內部單 / 可分配」**。
  （欄名一度叫「池子」，使用者反映看不懂 → 改成「內部單」，因為它直接對應
  店員在訂單頁看得到的【內部】xx 店 / RR- / OV- 單，看到就知道要走「轉單給客人」。）
