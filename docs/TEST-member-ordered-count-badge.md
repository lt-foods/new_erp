# member-ordered-count-badge 測試項目 — 「已訂購 / 已售出」改為醒目品牌徽章

**對應 UI 變更:**
- 新增 `apps/member/src/components/OrderedCount.tsx`（共用徽章：火焰 icon + 品牌色 pill，sm/md/lg 三個字級）
...
- `apps/member/src/app/shop/c/[id]/page.tsx`（標題旁總訂購量改用 OrderedCount lg；SKU 列表 + BuySheet 的 per-SKU「已售出 N」改品牌色 + 火焰 icon）
...
- [ ] `ordered_qty > 0`：價格右側出現玫瑰粉 pill「🔥已訂購 N 件」，數字粗體
- [ ] `ordered_qty = 0`：整顆徽章不渲染（不出現「已訂購 0 件」）
- [ ] 兩欄格線不破版：長數字（千分位）與瀏覽數直向堆疊仍對齊右緣
- [ ] 「僅剩 N 份」「已搶購一空」「倒數」chip 均不受影響

### 3.2 限時專區（`/shop/flash`）
- [ ] hero 卡片：徽章為 md 字級、靠右
- [ ] FlashRow 橫列：徽章為 sm 字級，與「X 筆訂單」「共 X 項」並存不擠壓

### 3.3 商品詳情頁（`/shop/c/[id]`）
- [ ] 標題右側總訂購量為 lg 徽章（取代原本灰底 rounded-lg）
- [ ] SKU 列表 per-SKU「已售出 N」為品牌色深玫瑰字 + 火焰 icon（維持「已售出」文案，scope 與卡片不同係刻意）
- [ ] BuySheet（選擇規格）內 per-SKU「已售出 N」同上、12px
- [ ] `ordered_qty = 0` 的品項不顯示已售出

### 3.4 一致性
- [ ] 四處徽章顏色 / icon / 顯示條件一致（皆出自 OrderedCount / FlameIcon，無各自複製的樣式）
