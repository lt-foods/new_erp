...
## 5. 「沒庫存就自動補一筆」—— 本案唯一的高風險設計

需求要的是：結帳當下發現帳上沒貨，照樣賣得掉。

**最終形態（Alex 2026-09-01 定案）：畫面上沒有任何補貨選項，帳上不夠的部分
由 RPC 自動補。** 只在銷售頁把「帳上只有 N 件、會自動補 M 件」寫出來，結帳一律過。

演進過程（三次改，理由都寫在 migration 檔頭）：
`20260901010010` 店員在畫面上勾、自己填數字 → `20260901030000` 夾成「剛好缺的量」
→ `20260901040000` 整個自動化、拿掉勾選與角色守衛。

**為什麼「不擋」是對的**：客人就站在櫃台前、東西在店員手上，貨**一定會**離開這家店。
系統擋下來不會讓貨留下，只會讓帳跟現實脫節 —— 店員繞過系統手收現金，那筆交易
就完全消失（沒有銷售、沒有扣庫存、沒有日結）。記成「+N 入帳 → −N 賣掉」才是
那一刻真正發生的事。同 CLAUDE.md 空中轉出庫 `p_allow_negative => TRUE` 的理由：
「貨實際上離開轉出店了，記下這筆出庫比拒絕記錄準確」。

**為什麼是 +N/−N 而不是負庫存**：`on_hand` 被全站好幾道閘門讀著（取貨閘門的實體
守衛、可分配量、現貨直配…），讓它變負數會誤擋別人的取貨。

**配套（自動化之後更重要，因為店員不會特別意識到）**：

1. **成本要帶**：`manual_adjust` 不帶成本時，若該倉 `avg_cost` 是 0，後面那筆
   `sale` 的 COGS 就是 0、毛利虛高。帶 `_current_cost_price` 當 fallback。
2. **`/pos/topups` 是唯一的訊號**。`on_hand` 淨變化 0，帳面上看不出差異，所以
   稽核只剩這份清單（同店連補 3 天以上會跳提示導到盤點）。**刪掉那支報表 ＝
   這個口就沒人看著了。**
3. **單頭留痕**：notes 寫「其中 N 件結帳時帳上沒有，系統自動補帳入庫」。
4. **`reason` 固定格式** `現場銷售即時入帳 <單號>（…）`，報表靠這個字串撈。
   改字串就會弄斷 `rpc_walkin_stock_topups`，兩邊要一起改。
5. **沒有角色守衛是刻意的**：一旦自動化，擋 `store_staff` ＝「店員結不了帳」，
   而結不了帳的後果就是這筆交易在系統上消失。操作人記在 movement 上。

**明確不做**：不碰減抵單（`rpc_create_inventory_deduction` 自己就強制
`on_hand - reserved >= qty`，而 Path D 的無條件豁免正是 2026-08-18 松山
「庫存 0 還能一直取貨」的元凶）；不做負庫存直售。
...
| 2 | 缺貨補庫存誰能按？ | 原本：**店長**。2026-09-01 改成**全自動、沒有按鈕**（「背後幫它自動補就好…也可以結單」）→ §5 |
