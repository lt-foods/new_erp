...
/**
 * 小幫手加單的畫面本體。整頁版在 /campaigns/order-entry，LINE 記事本的留言列點「團」
 * 會用 embedded 開在彈窗裡 —— 兩邊同一份，別為了彈窗再抄一份出去。
 *
 * embedded：彈窗自己有標題列與捲動容器，所以不畫 h1、不吃 p-6 / flex-1。
 * initialMemberTerm：第一位顧客的搜尋框預填（記事本把留言裡的會員編號帶過來）。
 * onCreated：送出成功後通知外層（彈窗用來重新整理留言列表）。
 */
export function OrderEntryView({
  campaignId, embedded = false, initialMemberTerm, onCreated,
}: {
  campaignId: number;
  embedded?: boolean;
  initialMemberTerm?: string;
  onCreated?: () => void;
}) {
  ...
  return (
    <div className={embedded ? "flex flex-col gap-4" : "flex flex-1 flex-col gap-4 p-6"}>
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            {!embedded && <h1 className="text-xl font-semibold">小幫手加單</h1>}
            {campaign ? (
          ...
          <div className="flex flex-col gap-3">
            {entries.map((e, i) => (
              <CustomerCard
                ...
                initialTerm={i === 0 ? initialMemberTerm : undefined}
                channelId={channelId}
...
function CustomerCard({
  entry, campaignId, channelId, campaignSkus, pickedMemberIds, initialTerm,
...
function CustomerSearch({
  channelId, value, pickedMemberIds, initialTerm, onPick,
}: {
  channelId: number | null;
  value: CustomerEntry;
  pickedMemberIds: Set<number>;
  // 外面帶進來的搜尋字（記事本的會員編號）。只當初值，之後使用者自己打
  initialTerm?: string;
  onPick: (patch: Partial<CustomerEntry>) => void;
}) {
  const [term, setTerm] = useState(initialTerm ?? "");
  // 有預填就直接把候選清單攤開 —— 彈窗加單的第一個動作一定是挑人，不用再點一下
  const [open, setOpen] = useState(!!initialTerm);
