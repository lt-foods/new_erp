...
//   不是「總倉內部怎麼處理這批貨」。
// ⚠️ 這兩行原本寫「**所以只有兩個答案**:同意退回要不要補貨」＋
//   「兩顆處理鈕 + 一行『第三個答案還沒做』的說明」——**兩句都已經是假的**：
//   Alex 2026-09-03 把第三個答案做出來了(reject_return,20260903000020 + 000200),
//   老闆 9/02 又把流程定成兩步。⛔ 不要照舊敘述改回單層兩顆。
// 顯示(現況):
//   - 這一筆的派出 / 實收 / 少收
//   - 第 1 步:不可逆紅框 → 這張單是誰派的(錢會怎麼算) → 兩顆(同意退／不同意退)
//   - 第 2 步(只有同意退才展開):這張單是誰派的(補貨/不補貨的差別) → 兩顆
//   - 一行「錢怎麼算」(只講無條件為真的部分) + 備註
//
...
export function TransferShortageResolveModal({
  ...
  // ⚠️ 最後那句原本寫「現在**兩顆都是「同意退回」**,備註一律選填」——
  //   那是單層兩顆時代的敘述,已經過期:現在是兩步、三個結果
  //   (同意退＋補貨／同意退＋不補貨／不同意退)。
  //   ⇒ 仍然為真的只有結論本身:**備註一律選填**(三個結果都沒有必填原因)。
  //   ⛔ 不要因為看到「兩顆」就把 accept 或必填欄位加回來。
  ...
  return (
    <Modal open onClose={onClose} title="處理店家少收的貨" maxWidth="max-w-3xl">
      ...
      <div className="mt-4 space-y-2">
        ...
        {/* 這一塊是「錢怎麼算」的說明（一律顯示）。
            ⚠️ 開頭那兩行原本寫「第三個答案『不同意退貨』**還沒做** ⇒ 刻意做成說明文字
              不是按鈕（老闆 2026-08-21 定，原因是總倉真的按不出這個結果）」——
              **已經過期**：Alex 2026-09-03 把它做成真的按鈕了（reject_return），
              現在它是第 1 步的第二顆。⛔ 不要照舊敘述把它改回一段死文字。
            ⭐ 下面那句是這裡唯一一句「系統會怎樣」的話,出處:
