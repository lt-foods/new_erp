...
//   來源（未綁 LINE）的訂單會一起搬到目標；目標（已綁 LINE）可以已有訂單，
//   只有當來源與目標在同一 (團, 頻道, 訂單類型) 都有 active 訂單時 RPC 才擋（會撞唯一索引）
      ...
      <div className="space-y-4 text-sm">
        <div className="rounded-md border border-amber-300 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/40">
          <div className="text-xs text-amber-800 dark:text-amber-300">
            ...
            <br />
            <b>來源（未綁 LINE）的訂單會一起搬到目標</b>；目標（已綁 LINE）會員<b>可以已有訂單</b>。唯一例外：兩筆會員在<b>同一團、同一頻道</b>都有進行中的訂單時無法合併（會撞單），需先把其中一筆的該訂單處理掉再合併。
          </div>
        </div>
