          ...
          receipts.map((r) => (
            <div key={r.event.id} className="receipt-single bg-white p-4 text-zinc-900">
              ...
              <div className="mb-3 grid grid-cols-2 gap-x-4 gap-y-1.5 text-base">
                ...
                <Field label="開團" value={r.order?.campaign?.name ?? "—"} />
              </div>

              <table className="mb-2 w-full border-collapse text-xl">
                <thead>
                  ...
                  {r.items.map((it) => {
                    ...
                    return (
                      <Fragment key={it.id}>
                        ...
                        {it.notes && (
                          <tr className="border-b border-zinc-200">
                            <td colSpan={4} className="px-2 pb-1 text-base italic text-zinc-600">
                              ↳ 備註：{it.notes}
                            </td>
                          </tr>
                        )}
                    ...
                    return (
                      <>
                        <tr className="border-t border-zinc-300">
                          <td colSpan={3} className="px-2 py-1 text-right text-base text-zinc-600">小計</td>
                          <td className="px-2 py-1 text-right font-mono">${sub}</td>
                        </tr>
                        {pct > 0 && (
                          <tr>
                            <td colSpan={3} className="px-2 py-1 text-right text-base text-zinc-600">− 折扣 {pct}%</td>
                            <td className="px-2 py-1 text-right font-mono">−${pctDed}</td>
                          </tr>
                        )}
                        {disc > 0 && (
                          <tr>
                            <td colSpan={3} className="px-2 py-1 text-right text-base text-zinc-600">− 折扣金額</td>
                            <td className="px-2 py-1 text-right font-mono">−${disc}</td>
                          </tr>
                        )}
                        <tr className="border-t-2 border-zinc-400 text-2xl font-bold">
                        ...
                        {walletPaid > 0 && (
                          <>
                            <tr>
                              <td colSpan={3} className="px-2 py-1 text-right text-base text-zinc-600">− 已用儲值金</td>
