...
/**
 * 店家釋出商品（互助交流板「我有庫存可提供」）在會員端的資料形狀。
 *
 * unit_price 只有 is_my_store=true 時才會有值 —— 跨店的金額後端就不放進
 * response（見 liff-api 的 listReleasedProducts），這裡拿到的一定是 null。
 */
...
export function releasedProductTitle(p: ReleasedProduct): string {
  const base = p.product_name ?? p.sku_code ?? "商品";
  return p.variant_name ? `${base} / ${p.variant_name}` : base;
}

/** 沒有商品圖時的暖色品牌底（和 CampaignCard 的回退一致，換成「釋出」箱子圖示） */
 ...
 */
export default function ReleasedProductCard({ item }: { item: ReleasedProduct }) {
